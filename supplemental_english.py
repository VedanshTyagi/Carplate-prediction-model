...
REGION_CODES = {i: k for k, v in REGION_CODES.items() for i in v}
...
import re
def find_code(plate):
    for i,j in GOVERNMENT_CODES.items():
        if re.sub(r"\d","",plate) == i[0] and int(plate[1:4]) in range(i[1][0],i[1][1]+1) and plate[6:]==i[2]:
            return j[-3:]
    return(0,0,0)
